...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] private PlayerInventory _player1Inventory;
    ...
    private int Player1StunEditing;
    private int Player2StunEditing;
    ...
    private void StartGame()
    {
        ...
        if (currentSceneName == "SampleScene")
        {
            ...
            PlayerUIEn("");
        bloodMagesticManager = GameObject.Find("BloodManager").GetComponent<BloodMagesticManager>();
    }
    ...
    private void SelectItemsForPlayers() //выборка
    {
        for (int indexer = 0; indexer < maxItems; indexer++)
        {
            CellInShop player1CellProperties = GameObject.Find("Item1" + indexer + "ToUse").GetComponent<CellInShop>();
            CellInShop player2CellProperties = GameObject.Find("Item2" + indexer + "ToUse").GetComponent<CellInShop>();
            if(_player1Inventory.data.Count > indexer)
            {
                player1CellProperties.ChangeCellType(_player1Inventory.data[indexer].effectTypeForThisItem, _player1Inventory.data[indexer]);
            }
            else
            {
                player1CellProperties.DisableThisItem();
            }
            if (_player2Inventory.data.Count > indexer)
            {
                player2CellProperties.ChangeCellType(_player2Inventory.data[indexer].effectTypeForThisItem, _player2Inventory.data[indexer]);
            }
            else
            {
                player2CellProperties.DisableThisItem();
            }
        }
    }
    ...
    public void AddItemPlayer(string PlayerName, ItemData itemData)
    {
        if (PlayerName == "Player1")
        {
            if (_player1Inventory.data.Count < MaxNumbersOfItem)
            {
                _player1Inventory.data.Add(itemData);
                UIManager.UpdateInventory(PlayerName);
            }

        }
        else
        {
            if (_player2Inventory.data.Count < MaxNumbersOfItem)
            {
                _player2Inventory.data.Add(itemData);
                UIManager.UpdateInventory(PlayerName);
            }
        }
    ...
    public void DeleteItemFromInventory(string PlayerName, ItemData data ,int full = 0)
    {
        if(full == 0)
        {
            if(PlayerName == "Player1")
            {
                int index = _player1Inventory.data.IndexOf(data);
                UIManager.UpdateInventory(PlayerName, true, index);
                _player1Inventory.data.Remove(data);
            }
            else
            {
                int index = _player2Inventory.data.IndexOf(data);
                UIManager.UpdateInventory(PlayerName, true, index);
                _player2Inventory.data.Remove(data);
            }
            
        }
    }

    public bool PlayerContainsItem(string PlayerName, ItemData data)
    {
        if(PlayerName == "Player1")
        {
            if (_player1Inventory.data.Contains(data))
            {
                return true;
            }
            else
        ...
        {
            if (_player2Inventory.data.Contains(data))
            {
                return true;
            }
            else
